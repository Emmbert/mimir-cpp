...
namespace psearch {
    ...
    ClientPublicMaterial generate_client_public_material(const CryptoContext& ctx,
        ...
        // CHANGED (version B): pass the ALREADY-BUILT lwe_to_rlwe_ksk directly,
        // instead of (lwe_sk, rlwe_gadget_sk_ksk) -- LWEToRGSWKeySwitchKey no
        // longer builds its own independent, redundant automorphism-key set.
        auto lwe_to_rgsw_ksk = std::make_shared<LWEToRGSWKeySwitchKey>(
            lwe_to_rlwe_ksk, *secret.rlwe_gadget_sk_rgsw);
